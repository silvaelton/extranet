...
module Protocol
  class Assessment < Support::Protocol::Assessment
    ...
    scope :by_document_type, ->(doc_type) {where(document_type_id: doc_type)}
    ...
    scope :by_recipient,     ->(interested) {where("recipient ilike ? ", "%#{interested}%")}
    scope :by_external,      ->(external_agency) {where("external_agency ilike ? ", "%#{external_agency}%")}
